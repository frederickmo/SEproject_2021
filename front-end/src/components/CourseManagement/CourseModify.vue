<template>
  ...
  <div v-if="this.modifystatus == 2">
    <div style="height: 12px" />
    <div style="display: flex">
      <a-button size="mini" style="margin-right: 10px" @click="this.status = 1"
        >学生管理</a-button
      >
    ...
    >
      ...
      <el-table-column align="right">
        ...
        <template #default="scope">
          <el-button size="mini" type="danger" @click="DropCourse(scope.row)"
            >点击退课</el-button
          >
        </template>
      </el-table-column>
  ...
  <div>
    <div>
      <a-modal v-model:visible="removestuVisible" v-on:ok="DropCourseok" simple>
        <template #title> 学生退课 </template>
        <div style="text-align: center">确认退课？</div>
      </a-modal>

      <a-modal
        v-model:visible="removeteaVisible"
        v-on:ok="DropmanageCourseok"
        simple
      >
        <template #title> 取消授课关系 </template>
        <div style="text-align: center">确认取消？</div>
      </a-modal>
...
export default {
  data() {
    return {
      ...
      stu: [],
      tea: [],
      removeteaVisible: false,
      removestuVisible: false,
      on: "",
      modifystatus: 1,
      status: 1,
      search: "",
  ...
  methods: {
    ...
    DropCourseok() {
      console.log(this.courseId + " " + this.stu.id);
      fetch(
        this.$URL +
    ...
    DropmanageCourseok() {
      if (localStorage.getItem("userId") != this.course_manage) {
        ...
        if (this.tea.id == this.id) {
          this.$notification.success("无法取消责任教师");
          //this.$router.go(-1)
        } else {
          console.log(this.courseId, this.tea.id);
          fetch(
            this.$URL +
